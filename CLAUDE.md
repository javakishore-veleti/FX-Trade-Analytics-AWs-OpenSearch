...
- **`deploy/helm/`** — Helm charts for EKS deployment. One per service plus an umbrella:
  - `fx-masterdata/` — Deployment + Service + ServiceAccount **+ pre-upgrade Liquibase Job** (the prod migration topology — Job runs `--spring.profiles.active=postgres,migrate`, exits via `MigrationRunner`; service Pods then boot with `postgres,no-migrate` so Liquibase is OFF and there's no `DATABASECHANGELOGLOCK` contention across N pods).
  - `fx-trade-service/` — REST + multi-region search proxy. ConfigMap-style env injection of `FX_OPENSEARCH_SOURCE_*` + `SPRING_KAFKA_BOOTSTRAP_SERVERS`.
  - `fx-risk-service/` — Kafka consumer worker. Actuator-only Service.
  - `fx-opensearch-indexer/` — Kafka consumer + SigV4-signed OpenSearch writes. Same env-injection pattern as trade-service.
  - `fx-platform/` — umbrella chart depending on the four above. `helm install fx ./deploy/helm/fx-platform -f values-prod.yaml` brings up the whole stack in one go. Per-service charts are also installable standalone.
  - All charts are IRSA-ready: set `serviceAccount.annotations.eks\.amazonaws\.com/role-arn` to use IAM roles via the projected SA token; otherwise fall back to `aws.credentialsSecretName` for static keys.
  - See `deploy/helm/README.md` for the install + auth + migration-pattern doc.
